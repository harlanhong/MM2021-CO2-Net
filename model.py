...
class Non_Local_Block(torch.nn.Module):
    def __init__(self, input_dim, output_dim, dropout_ratio):
        ...
        self.f = nn.Sequential(
            nn.Conv1d(input_dim, output_dim, 1, padding=0),nn.LeakyReLU(0.2),nn.Dropout(dropout_ratio))
        self.g = nn.Sequential(
            nn.Conv1d(input_dim, output_dim, 1, padding=0),nn.LeakyReLU(0.2),nn.Dropout(dropout_ratio))
        self.z = nn.Sequential(
            nn.Conv1d(input_dim, input_dim, 1, padding=0),nn.LeakyReLU(0.2),nn.Dropout(dropout_ratio))
        self.attention = nn.Sequential(nn.Conv1d(input_dim, 512, 3, padding=1),
                                       nn.LeakyReLU(0.2),
    ...
    def forward(self, feat):
        b,c,n = feat.shape
        relations = torch.softmax(torch.bmm(self.f(feat).transpose(-1,-2),self.g(feat)),-1)
        z = self.z(feat).transpose(-1,-2)
        # attn = feat.mean(1,keepdim=True)
        attn_feat = []
        for i in range(n):
            attn_feat.append((relations[:,i,:].unsqueeze(-1)*z).mean(1))
        relation_feat = torch.stack(attn_feat,1)
        x_atn = self.attention(relation_feat.transpose(-1,-2))
        # x_atn = relation_feat.mean(-1,keepdim=True).transpose(-1,-2)
        return x_atn #b,n,c
...
class SE_Attention(torch.nn.Module):
    def __init__(self,n_featue,n_class,**args):
        super().__init__()
        embed_dim=1024
        reduce_ratio=16
        self.channel_avg=nn.AdaptiveAvgPool1d(1)
        self.conv=nn.Sequential(nn.Conv1d(embed_dim,embed_dim//reduce_ratio,1),nn.ReLU(),nn.Conv1d(embed_dim//16,embed_dim,1),nn.Sigmoid())
    ...
    def forward(self,vfeat,ffeat):
        ...
        att=self.conv(channelfeat)
        filter_feat=att*vfeat
