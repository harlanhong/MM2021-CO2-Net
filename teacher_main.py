...
if __name__ == '__main__':
   ...
   student_model = getattr(model,args.use_model)(dataset.feature_size, dataset.num_class,opt=args).to(device)
   teacher_model = getattr(model,args.use_model)(dataset.feature_size, dataset.num_class,opt=args).to(device)

   if args.pretrained_ckpt is not None:
      student_model.load_state_dict(torch.load(args.pretrained_ckpt))
   ...
   for itr in tqdm(range(args.max_iter)):
      loss = train(itr, dataset, args, student_model, optimizer, logger, device,teacher_model=teacher_model)
      total_loss+=loss
      if itr % args.interval == 0 and not itr == 0:
         print('Iteration: %d, Loss: %.5f' %(itr, total_loss/args.interval))
         total_loss = 0
         torch.save(teacher_model.state_dict(), './ckpt/last_' + args.model_name + '.pkl')
         iou,dmap = test(itr, dataset, args, teacher_model, logger, device,pool)
         if 'Thumos' in args.dataset_name:
            cond=sum(dmap)>sum(max_map)
         else:
            cond=np.mean(dmap)>np.mean(max_map)
         if cond:
            torch.save(teacher_model.state_dict(), './ckpt/best_' + args.model_name + '.pkl')
            max_map = dmap
      ...
      update_ema_variables(student_model, teacher_model, args.ema_decay, itr+1)
         # print('Detection MAX map @ %2f = %f, map @ %2f = %f, map @ %2f = %f, map @ %2f = %f, map @ %2f = %f' %(iou[0], max_map[0],iou[1], max_map[1],iou[2], max_map[2],iou[3], max_map[3],iou[4], max_map[4]))
      # if itr % int(args.max_iter/3) == 0 and not itr == 0:
      #    for param_group in optimizer.param_groups:
      #       param_group['lr'] = lrs[int(itr/int(args.max_iter/3))]
         # print('Change Lr====================================')
